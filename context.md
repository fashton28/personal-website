...
- **2026-06-29** — **Fixed blog lists not rendering.** Root cause was CSS, not markdown: Tailwind's preflight base reset sets `list-style: none` on `ul`/`ol`, and `.prose` only set `padding-left` + a `::marker` color, so bullets/numbers never showed. Added `.prose ul { list-style-type: disc }`, `.prose ol { list-style-type: decimal }`, and `.prose ul ul { list-style-type: circle }` in `globals.css`. Also cleaned up `posts/agentic-2026.md` where several paragraphs sat directly under a list with no blank line — they were being absorbed into the last `<li>` (lazy continuation). Added the missing blank lines so trailing copy renders as its own `<p>`. Build verified green.
- **2026-06-22** — **Cmd+K palette is now a content search engine**, not a route menu. It surfaces actual content: **Projects** (from `data/projects.ts` — searchable by title, summary, tags) and **Writing** (from `getAllPostsMeta()`), plus a **Links** group (socials + copy-email). Selecting a project deep-links to its reel panel via `/projects#<anchorId>`; selecting a post opens `/writing/<slug>`. Posts need `fs`, so `layout.tsx` (server) calls `getAllPostsMeta()` and passes `posts` to `<GlobalCommand posts={...} />` (client). Added `group?: string` to `CommandActionItem`; `command-palette.tsx` now renders one `CommandGroup` per `group` (ordered Projects → Writing → Links) instead of the old action-based Navigation/Links split — so future content types just add more grouped items. Removed the old section-scroll + route nav items. Added hash deep-link handling to `projects/page.tsx` (`#anchorId` → scroll that panel + set active, also on `hashchange`). Dropped per-item keyboard shortcuts (they were route-specific). cmdk filters on label+keywords as before.
